...
remain exclusively in the hosted release job.
...
The separate immutable `OpenCore-autoboot-v2.qcow2` boot disk disables the picker
and preboot hotkeys for unattended startup. Its SHA-256 is part of the validation
identity and the host's startup checksum list; the original macOS/toolchain image
is unchanged. A rollout trial exposed a VM waiting at the picker with the older
interactive boot disk, so that attempt is excluded from successful trials.
