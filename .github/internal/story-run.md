...
The runner lives in this app checkout under `Tools/Story`. Run the commands
below through `scripts/winnow-story`; no separate story repository is needed.
It pins the same published library version as the app. Run `scripts/ci-story`
for offline tests and wrapper checks before starting the manual journey.
